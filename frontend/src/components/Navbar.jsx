...
const Navbar = () => {
  ...
  const {getProductCount} = useContext(ShopContext);

  return (
    <div className='flex items-center justify-between  py-5 font-medium'>
      <Link to='/'> 
           <img src={assets.logo} className='w-36' alt='logo image'></img>
      </Link>
      <ul className='max-sm:hidden flex gap-5 text-sm text-gray-700'>
      ...
      <div className='flex items-center  gap-6'>
          ...
          <Link to='/orders' className='relative'>
                  ...
                  <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-black text-white aspect-square rounded-full text-[8px]'>{getProductCount()}</p>
          </Link>
          <img onClick={()=>{
            setVisible(true);
          }} src={assets.menu_icon} className="w-5 cursor-pointer sm:hidden" alt="menu-icon"/>
        </div>
