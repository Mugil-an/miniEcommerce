  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
      ...
      <div className='min-w-60'>
        ...
        <div className={`border border-gray-300 py-3 pl-5 mt-6 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
      ...
      <div className='flex-1' >
        <div className='text-base sm:text-2xl mb-4 flex flex-row justify-between'>
          <Title text1={'ALL'} text2={'COLLECTIONS'}/>
          {/*Product sort */}
          <select onChange ={(e)=>setSortType(e.target.value)}className='text-sm px-2 border-2 border-gray-300'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
           {filterProducts.map((item,index)=>(
            <ProductsItems key={index} name={item.name} id={item._id} price={item.price} images={item.images} />
           ))}
        </div>
      </div>
    </div>
